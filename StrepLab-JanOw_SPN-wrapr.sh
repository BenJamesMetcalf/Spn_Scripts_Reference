...
#qsub -sync y -q short.q -t 1-$(cat $out_jobCntrl/job-control.txt | wc -l) -cwd -o "$out_qsub" -e "$out_qsub" ./StrepLab-JanOw_SPN-Typer.sh $out_jobCntrl
...
qsub -sync y -q all.q,short.q -t 1-$(cat $out_jobCntrl/job-control.txt | wc -l) -cwd -o "$out_qsub" -e "$out_qsub" -l h_vmem=32G,h_rt=2:00:00 -pe smp 8 ./StrepLab-JanOw_SPN-Typer.sh $out_jobCntrl
...
do
    ...
    cat $final_outDir/TABLE_Isolate_Typing_results.txt >> $final_result_Dir/TABLE_SPN_"$batch_name"_Typing_Results2.txt
